...
t_line *upper_hull( t_point *points, int num_point, t_point p1, t_point pn, t_line *lines, int *num_line, int *capacity) {
	...
	separate_points(points, num_point, p1, p_max, s1, s2, &s1_1, &s1_2);
	lines = upper_hull(s1, s1_1, p1, p_max, lines, num_line, capacity);

	s1 = (t_point*)realloc(s1, sizeof(t_point) * num_point);
	assert(s1 != NULL);
	s2 = (t_point*)realloc(s2, sizeof(t_point) * num_point);
	assert(s2 != NULL);

	separate_points(points, num_point, p_max, pn, s1, s2, &s1_1, &s1_2);
	lines = upper_hull(s1, s1_1, p_max, pn, lines, num_line, capacity);
